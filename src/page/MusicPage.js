...
import lzPlay from '../utils/lzPlayCore';

import Icon from '../components/Icon';
import Tabs from '../components/Tabs';
import CirclePorgress from '../components/CirclePorgress';
import DragPorgress from '../components/DragPorgress';
...
export default class MusicPage extends Component{
  constructor(props) {
    ...
    store.subscribe((state,action) =>{
      ...
      this.state.playIdx = data.counter.idx;
      this.state.playList = data.counter.list;
      this.resetPlayUrl();
      }
    )
  }
  ...
  resetPlayUrl(){
    this.state.playUrl = 'http://192.168.0.5/vd/' + this.state.playList[this.state.playIdx] + '.mp3';
    lzPlay.setVidioUrl(this.state.playUrl);
  }
    ...
    return <div className="music-container">
            ...
            <div className="big-container transiton-move" style={{height:this.state.bheight + "px"}}>
              <div className="img-blur-container">
                <img src={require('../img/lrc_control_img_default.png')} />
              </div>
  ...
  componentDidMount(){
    ...
    lzPlay.initVidio({
      ...
      'pause': function(){
        self.state.palyHandle && clearInterval(self.state.palyHandle);
        self.state.playState = false;
        // self.state.playIcon = 'bofang';
        self.setState(self.state);
      },
      'play': function(){
        self.state.palyHandle && clearInterval(self.state.palyHandle);
        self.state.playState = true;
      ...
      'ended':function(){
        if(self.state.playList.length > 0){
          if(self.state.playIdx + 1 >= self.state.playList.length){
            self.state.playIdx = 0;
          }else{
            self.state.playIdx++;
          }
        }
